...
abstract class BasesfGuardUser extends BaseObject implements Persistent
{
        ...
        if (null === $this->collsfGuardUserProfiles || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collsfGuardUserProfiles) {
                ...
                if (null !== $criteria) {
                    ...
                    $collsfGuardUserProfiles->getInternalIterator()->rewind();
    ...
    {
        $sfGuardUserProfilesToDelete = $this->getsfGuardUserProfiles(new Criteria(), $con)->diff($sfGuardUserProfiles);


        $this->sfGuardUserProfilesScheduledForDeletion = $sfGuardUserProfilesToDelete;

        foreach ($sfGuardUserProfilesToDelete as $sfGuardUserProfileRemoved) {
    ...
    public function addsfGuardUserProfile(sfGuardUserProfile $l)
    {
        ...
        if (!in_array($l, $this->collsfGuardUserProfiles->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddsfGuardUserProfile($l);

            if ($this->sfGuardUserProfilesScheduledForDeletion and $this->sfGuardUserProfilesScheduledForDeletion->contains($l)) {
                $this->sfGuardUserProfilesScheduledForDeletion->remove($this->sfGuardUserProfilesScheduledForDeletion->search($l));
            }
        }
    ...
    public function setsfGuardUserPermissions(PropelCollection $sfGuardUserPermissions, PropelPDO $con = null)
    {
        ...
        $this->sfGuardUserPermissionsScheduledForDeletion = clone $sfGuardUserPermissionsToDelete;
    ...
    public function addsfGuardUserPermission(sfGuardUserPermission $l)
    {
        ...
        if (!in_array($l, $this->collsfGuardUserPermissions->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            ...
            if ($this->sfGuardUserPermissionsScheduledForDeletion and $this->sfGuardUserPermissionsScheduledForDeletion->contains($l)) {
                $this->sfGuardUserPermissionsScheduledForDeletion->remove($this->sfGuardUserPermissionsScheduledForDeletion->search($l));
            }
        }
    ...
    public function setsfGuardRememberKeys(PropelCollection $sfGuardRememberKeys, PropelPDO $con = null)
    {
        ...
        //we can not just set a PK to NULL in the lines below. We have to store
        //a backup of all values, so we are able to manipulate these items based on the onDelete value later.
        $this->sfGuardRememberKeysScheduledForDeletion = clone $sfGuardRememberKeysToDelete;
    ...
    public function addsfGuardRememberKey(sfGuardRememberKey $l)
    {
        ...
        if (!in_array($l, $this->collsfGuardRememberKeys->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            ...
            if ($this->sfGuardRememberKeysScheduledForDeletion and $this->sfGuardRememberKeysScheduledForDeletion->contains($l)) {
                $this->sfGuardRememberKeysScheduledForDeletion->remove($this->sfGuardRememberKeysScheduledForDeletion->search($l));
            }
        }
