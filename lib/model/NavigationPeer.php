<?php



/**
 * Skeleton subclass for performing query and update operations on the 'navigation' table.
 *
 * 
 *
 * You should add additional methods to this class to meet the
 * application requirements.  This class will only be generated as
 * long as it does not already exist in the output directory.
 *
 * @package    propel.generator.lib.model
 */
class NavigationPeer extends BaseNavigationPeer {
  
    public static function disableAllChildren($parentId)
    {
        $con = Propel::getConnection();
        $c = new Criteria();
        $c->add(self::PARENT_TREE_ID, $parentId);
        $c1 = new Criteria();
        $c1->add(self::DISPLAY, 0);
        BasePeer::doUpdate($c, $c1, $con);
    }
    
    public static function getByRoute($route)
    {
        $c = new Criteria();
        $c->add(self::ROUTE, $route);
        return self::doSelectOne($c);
        if ($rs) {
            return $rs[0];
        }
        return null;
    }

} // NavigationPeer
